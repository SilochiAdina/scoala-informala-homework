...
public class PurchaseManager {
    ...
    private static void processPurchase(Product product, Customer customer){
        String errorMessage = checkAndReturnMessage(product, customer);
        if(errorMessage != null) {
            System.out.println("The purchase could not be processed. "+errorMessage);
        } else {
            product.setQuantity((int) (Product.getQuantity() - 1));
            customer.setBalance(customer.getBalance() - product.getPrice());
            System.out.println("Purchase processed successfully. "+
                    customer.getName() + " purchased " + product.getName());
        }
    }

    private static String checkAndReturnMessage(Product product, Customer customer) {
        ...
        if(isAccessRestricted(product.getAgeRestriction(), customer.getAge())) {
            return "You are too young to buy this product!";
        }
        return null;
    }
    ...
    private static boolean isExpired(Product product) {
        if(product instanceof IExpirable) {
            ...
            return LocalDate.now().isAfter(expirable.getexpirationDate());
        }
        return false;
    }

    private static boolean isAccessRestricted(String ageRestriction, int age) {
        switch (ageRestriction) {
            case Product.RESTRICTION_ADULT:
                return age<18;
            case Product.RESTRICTION_TEEN:
                return age<14;
            case Product.RESTRICTION_NONE:
            default:
                return false;
        }
    }
